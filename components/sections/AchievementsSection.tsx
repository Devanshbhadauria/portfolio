...
export const AchievementsSection: React.FC = () => {
  return (
    <section className="py-20 bg-[#0a0d15] relative border-t border-slate-800/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {ACHIEVEMENTS.map((item) => (
            <Card key={item.id} className="flex flex-col justify-between text-center items-center space-y-4">
              <div className="w-12 h-12 rounded-xl bg-slate-900 border border-slate-700 flex items-center justify-center text-emerald-400 font-mono font-bold text-xl shadow-inner">
                {item.iconName === 'award' && '🏆'}
              ...
              <div>
                ...
                <div className="text-xs font-mono text-cyan-400 font-semibold mt-0.5">{item.organization}</div>
              </div>
